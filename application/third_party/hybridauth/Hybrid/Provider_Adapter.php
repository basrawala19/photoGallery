	...
	*/
	function login()
	{
		...
		foreach( Hybrid_Auth::$config["providers"] as $idpid => $params ){
			//Hybrid_Auth::storage()->delete( "provider" );
		...
		$HYBRID_AUTH_URL_BASE = "http://localhost/codeigniter-2.2-stable/index.php/hauth/endpoint" ;
		...
		Hybrid_Logger::debug($HYBRID_AUTH_URL_BASE);
		# for default HybridAuth endpoint url hauth_login_done_url
		# 	auth.done   required  the IDp ID
		$this->params["login_done"]  = $HYBRID_AUTH_URL_BASE . ( strpos( $HYBRID_AUTH_URL_BASE, '?' ) ? '&' : '?' ) . "hauth.done={$this->id}";

		Hybrid_Logger::debug("setting hatim provider".$providerId);
		//Hybrid_Auth::storage()->set( "provider",$this->id );
