...
class Hybrid_Providers_Facebook extends Hybrid_Provider_Model
{
	...
	//public $scope = "public_profile email user_about_me user_birthday user_hometown user_website";
	public $scope = "email" ;
	/**
	...
	*/
	function getUserProfile()
	{
		...
		try{
			$data = $this->api->api('/me?locale=en_US&fields=name,email,first_name,last_name,link');
		}
		...
		/*foreach ($data as $key => $value) {
			Hybrid_Logger::debug("hatim {$key} => {$value}");
			# code...
		}*/

		# store the user profile.
		$this->user->profile->provider = $this->providerId ;
		$this->user->profile->external_id    = (array_key_exists('id',$data))?$data['id']:"";
		//$this->user->profile->displayName   = (array_key_exists('name',$data))?$data['name']:"";
		$this->user->profile->firstName     = (array_key_exists('first_name',$data))?$data['first_name']:"";
		$this->user->profile->lastName      = (array_key_exists('last_name',$data))?$data['last_name']:"";
		$this->user->profile->photoURL      = "https://graph.facebook.com/" . $this->user->profile->external_id . "/picture?width=150&height=150";
	...
	*/
	function getUserContacts()
	{
		...
		foreach( $response["data"] as $item ){
			...
			Hybrid_Logger::debug("taha {$uc->identifier}");
			$contacts[] = $uc;
		}
